from mod.utils import make_dir_if_not_exists, load_from_pickle, load_from_json, MIN_POS_SAMPLES_THRESHOLD
import os
import csv
from data_preprocessing import data_preprocess
from multitask_bert_entitity_classifier import bert_classifier
import logging
logging.basicConfig()
...
for taskname, (data_in_file, processed_out_file) in task_type_to_datapath_dict.items():
	...
	if not os.path.exists(results_file) or REDO_FLAG:
		...
		save_dir = 'saved_models/multitask_bert_entity_classifier_fixed/{taskname}_8_epoch_32_batch_multitask_bert_model'
		if RETRAIN_FLAG:
			multitask_bert_cmd += " -rt"
		logging.info(f"Running: {multitask_bert_cmd}")
		try:
			# def bert_classifier(data_file, task, save_directory, output_dir, retrain, batch_size, n_epochs):
			bert_classifier(processed_out_file,taskname,save_dir,output_dir,retrain=True)
