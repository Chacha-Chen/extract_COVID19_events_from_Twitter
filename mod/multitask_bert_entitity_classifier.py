...
class TokenizeCollator():
	...
	def __call__(self, batch):
		...
		# print(input_ids)
		# First extract the indices of <E> token in each sentence and save it in the batch # TODO what is this????
		...
		## input_ids [batch_size, 68] [8,2] label 8 dim 10 subtasks,
	...
	for text, chunk, chunk_id, chunk_start_text_id, chunk_end_text_id, tokenized_tweet, tokenized_tweet_with_masked_chunk, subtask_labels_dict in data:
		for subtask in subtasks:
			subtasks_data[subtask].append((text, chunk, chunk_id, chunk_start_text_id, chunk_end_text_id,
										   ...
										   subtask_labels_dict[subtask][0],
										   subtask_labels_dict[subtask][1]))
...
def bert_classifier(data_file, task, save_directory, output_dir, retrain, batch_size=32, n_epochs=8):
	...
	if retrain:
		...
		new_embedding_weight = torch.cat((model.bert.embeddings.word_embeddings.weight.data,new_embeddings), dim=0)
	...
	train_data, dev_data, test_data = split_multitask_instances_in_train_dev_test(data) ## list of 8 tuples
	...
	if retrain:
		...
		scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps=0, num_training_steps=total_steps) ## learning rate decrease linearly from the initial
		...
		for epoch in range(epochs):
			...
			for step, batch in enumerate(pbar): #batch dict{}4
