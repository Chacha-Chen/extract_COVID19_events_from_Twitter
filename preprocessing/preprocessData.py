...
def getValidSubtaskList(
        raw_subtask_list,gold_label_stats_for_q_tag_dict):
    subtask_list = []
    for subtask in raw_subtask_list:
        # 1 here for positive
        if gold_label_stats_for_q_tag_dict[subtask][1]  >= const.MIN_POS_SAMPLES_THRESHOLD:
            subtask_list.append(subtask)
    return subtask_list

    
def preprocessDataAndSave(event):
    ...
    for data_item_idx, data_item in enumerate(dataset):
        ...
        for q_tag, q_key in question_tag_and_key_list:
            ...
            instance_list_for_subtask_dict[q_tag] += [
                (tweet_text, tweet_text_tokenized, *raw_instance)
    ...
    for instance in all_instance_list:
        ...
        tweet_text = instance[0]
        chunk = instance[2]
        tweet_text_tokenized_with_masked_chunk = instance[4]
        ...
        raw_input_text_and_label_list.append((tweet_text, input_text, subtask_label_dict))
...
def splitDatasetIntoTrainDevTest(raw_input_text_and_label_list, train_ratio=0.6, dev_ratio=0.15):
    ...
    for tweet_text, _, _ in raw_input_text_and_label_list:
    ...
    for tweet_text, input_text, subtask_label_dict in raw_input_text_and_label_list:
        item = (input_text, subtask_label_dict)
